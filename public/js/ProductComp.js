Vue.component('products', {
    ...
    mounted(){
        this.$parent. postJson(`${API + this.catalogUrl}`, {table: 'products', req: 'get_products'})
            .then(data => {
                for(let el of data){
    ...
    template: `
        <div class="products">
            <product ref="refref" v-for="item of filtered" :key="item.id_product" :img="catalogImg+item.product_img" :product="item"></product>
        </div>
    `
});
...
<!--                    <button class="buy-btn" @click="$root.$refs.cart.addProduct(product.product_name)">Купить</button>-->
